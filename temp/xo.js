		...
		if (argument === 'play') {
			if (players.length == 0) {
				...
				message.channel.send(
					`Player 1: ${players[0].user.username}, waiting for second user. Type \`!xo play\` to play`,
				);
			...
			if (players.length == 1) {
				...
				collector.on('collect', (m) => {
					...
					if (
						...
						console.log(players);
						message.channel.send(
							`Player 1: ${players[0].user.username}, Player 2: ${players[1].user.username}`,
						);
						collector.stop();
					}
				});
				...
				collector.on('end', (collected) => {
					...
					if (players.length < 2) {
						message.channel
							.send(
								`Guess no one want to play with ${players[0].user.username} :frowning:`,
							)
							.then(() => {
								players = new Array();
							});
